...
public class EditarFuncionarioServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        func.setNumMatricula(Integer.parseInt(request.getParameter("numMatricula")));

        if (funcDao.editaFuncionario(func)) {
            response.sendRedirect("logado/listaTodosFuncionarios.jsp");
        } else {
            response.sendRedirect("logado/editarFuncionario.jsp");
        }
    }
}